<div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500">Administra los estudiantes por clase</p>
        </div>
        @if($claseId)
            <div class="flex gap-2">
                <button wire:click="openImport"
                        class="border border-[#000b60] text-[#000b60] px-4 py-2 rounded-lg font-semibold flex items-center gap-2 hover:bg-blue-50 transition">
    ...
    <div class="bg-white rounded-xl shadow p-4 mb-6 flex items-center gap-4">
        <span class="material-symbols-outlined text-[#000b60]">school</span>
        <div class="flex-1">
            <label class="block text-xs font-bold text-[#000b60] mb-1">Clase</label>
            <select wire:model.live="claseId"
                    class="w-full md:w-80 border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#000b60] text-sm">
        ...
        @if($claseId)
            <div class="text-right text-sm text-gray-500">
                <span class="font-bold text-2xl text-[#000b60]">{{ $estudiantes->count() }}</span><br>
                estudiante(s)
            </div>
        @endif
    </div>
    ...
    @if(!$claseId)
        ...
        <div class="bg-white rounded-xl shadow flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p class="mt-3 font-semibold text-gray-500">Selecciona una clase para gestionar sus estudiantes</p>
        </div>
    ...
    @elseif($estudiantes->isEmpty())
        ...
        <div class="bg-white rounded-xl shadow flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p class="mt-3 font-semibold text-gray-500">No hay estudiantes en esta clase</p>
            <p class="text-sm mt-1">Agrega individualmente o importa desde Excel</p>
            <div class="flex gap-3 mt-5">
                <button wire:click="openImport"
                        class="border border-[#000b60] text-[#000b60] px-4 py-2 rounded-lg font-semibold text-sm hover:bg-blue-50 transition">
                    Importar Excel
                </button>
                <button wire:click="openCreate"
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($estudiantes as $i => $e)
                        <tr class="hover:bg-[#f3faff] transition">
                            ...
                            <td class="px-6 py-4 font-mono font-semibold text-[#000b60]">{{ $e->carnet }}</td>
                            <td class="px-6 py-4 font-semibold">{{ $e->nombre }}</td>
                            <td class="px-6 py-4 text-gray-500">{{ $e->correo ?? '—' }}</td>
                            <td class="px-6 py-4">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input wire:model="carnet"
                               ...
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#000b60] @error('carnet') border-red-400 @enderror">
                        @error('carnet')
                    ...
                    <div>
                        ...
                        <input wire:model="correo"
                               ...
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#000b60] @error('correo') border-red-400 @enderror">
                        @error('correo')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    @if($showImportModal)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6">
                ...
                <div class="flex justify-between items-center mb-5">
                    <h2 class="text-xl font-black text-[#000b60]">Importar desde Excel</h2>
                ...
                {{-- Formato requerido --}}
                <div class="bg-[#e6f6ff] rounded-lg p-4 mb-5">
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-xs font-bold text-[#000b60] uppercase tracking-wide">Formato requerido de columnas</p>
                        <a href="{{ route('estudiantes.plantilla') }}"
                           class="flex items-center gap-1 text-xs font-semibold text-[#000b60] bg-white border border-[#000b60] px-2.5 py-1 rounded-lg hover:bg-blue-50 transition"
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-xs border-collapse">
                            <thead>
                                <tr class="text-[#000b60]">
                                    <th class="border border-blue-200 bg-blue-100 px-3 py-1.5 text-left">No.</th>
                                    <th class="border border-blue-200 bg-blue-100 px-3 py-1.5 text-left">Carné</th>
                                    <th class="border border-blue-200 bg-blue-100 px-3 py-1.5 text-left">Estudiante</th>
                                    <th class="border border-blue-200 bg-blue-100 px-3 py-1.5 text-left">Correo Electrónico</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="text-gray-600">
                                    <td class="border border-blue-100 px-3 py-1.5">1</td>
                                    <td class="border border-blue-100 px-3 py-1.5">202300001</td>
                                    <td class="border border-blue-100 px-3 py-1.5">Juan Pérez García</td>
                                    <td class="border border-blue-100 px-3 py-1.5">juan@uni.edu</td>
                                </tr>
                                <tr class="text-gray-600">
                                    <td class="border border-blue-100 px-3 py-1.5">2</td>
                                    <td class="border border-blue-100 px-3 py-1.5">202300002</td>
                                    <td class="border border-blue-100 px-3 py-1.5">María López</td>
                                    <td class="border border-blue-100 px-3 py-1.5">maria@uni.edu</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <p class="text-xs text-gray-500 mt-2">Formatos aceptados: <strong>.xlsx</strong>, <strong>.xls</strong>, <strong>.csv</strong> · Máx. 10 MB</p>
                </div>
                ...
                <div>
                    ...
                    <input wire:model="archivo"
                           type="file"
                           accept=".xlsx,.xls,.csv"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#000b60] @error('archivo') border-red-400 @enderror">
                    @error('archivo')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
